...
class Certificado(models.Model):
    paciente = models.ForeignKey(Paciente, on_delete=models.CASCADE, verbose_name="Paciente", related_name="certificados")
    doctor = models.ForeignKey(Doctor, on_delete=models.PROTECT, verbose_name="Doctor", related_name="certificados")
    ...
    detalles = models.TextField(verbose_name="Detalles del Certificado", null=True, blank=True)
    firmado = models.BooleanField(default=False, verbose_name="Firmado")
    archivo_pdf = models.FileField(upload_to='certificados/', verbose_name="Certificado en PDF", null=True, blank=True)

    def __str__(self):
        return f"Certificado de {self.paciente.nombre_completo} emitido por {self.doctor.nombre_completo}"
